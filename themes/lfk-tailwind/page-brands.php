...
<div class="lfk-brand-directory-page" role="main">
	<div class="lfk-shell">
		<div class="lfk-brand-visual-grid">
			<?php foreach ( $brand_cards as $index => $card ) : ?>
				<?php
				...
				<section class="lfk-brand-visual-card<?php echo $is_large ? ' is-large' : ''; ?>">
					<a href="<?php echo esc_url( get_term_link( $brand ) ); ?>">
						<img<?php echo $is_priority_image ? ' class="skip-lazy" data-no-lazy="1"' : ''; ?> src="<?php echo esc_url( lfk_remote_upload_url( $card['image'] ) ); ?>" alt="<?php echo esc_attr( $card['alt'] ); ?>" width="<?php echo $is_large ? '415' : '300'; ?>" height="<?php echo $is_large ? '415' : '300'; ?>" loading="<?php echo $is_priority_image ? 'eager' : 'lazy'; ?>"<?php echo $is_priority_image ? ' fetchpriority="high"' : ''; ?>>
